...
class TestLoginCourier:
    ...
    @allure.title('Проверка на то, что система вернёт ошибку, если не указать пароль')
    ...
    @allure.title('Проверка на то, что система вернёт ошибку, если не указать логин')
    def test_login_without_login(self, courier_methods):
        log_pass = courier_methods.register_new_courier_and_return_login_password()
        result = courier_methods.login_courier('', log_pass[1])
        assert result[0] == 400 and {'code': 400, 'message': 'Недостаточно данных для входа'} == result[1]

    @allure.title('Проверка на то, что система вернёт ошибку, если неправильно указать пароль')
    ...
    @allure.title('Проверка на то, что система вернёт ошибку, если неправильно указать логин')
